    ;──═◙ "Drone Patrol", Steve Engelhardt, 3/8/2023
...
MainLoop
    ...
    ;──═◙ Health Check
    ; Battery will only drop if your health is less than 15, and will only drop 3 times.
    ;if battery_flag>1 then goto skip_batterydrop
    ;if healthMed=$00 && healthLo<$75 then battery_flag=1 else battery_flag=0 ; Drop Battery if your health is below the specified level in HealthLo
skip_batterydrop
    ;if battery_ypos>99 && battery_flag=1 then battery_ypos=0:battery_xpos=(rand&127)+10 ; Reposition X value of battery drop after each fall
    ;if battery_ypos>95 then battery_counter=battery_counter+1
    ;if battery_counter>4 then battery_flag=2:battery_counter=0
...
skiphealthcheck
    ...
    if animate1>3 then animate1=0 : tank1_xpos=tank1_xpos+1: tank2_xpos=tank2_xpos+1: tank3_xpos=tank3_xpos+1
    animate2=animate2+1
    if animate2>4 then animate2=0
    if animate2=3 then animate3=animate3+1:animate4=animate4+1:animate5=animate5+1
    if animate3>3 && tank1_flag=1 then tank1_flag=0:dec scorea=scorea+100:tank1_xpos=200:crosshair_x_offset_1=200
    if animate4>3 && tank2_flag=1 then tank2_flag=0:dec scorea=scorea+100:tank2_xpos=200:crosshair_x_offset_1=200
    if animate5>3 && tank3_flag=1 then tank3_flag=0:dec scorea=scorea+100:tank3_xpos=200:crosshair_x_offset_1=200
    ...
    if fire_debounce_1=1 && frame_counter=2 && laserLo<>0 then dec laserLo=laserLo-1
...
skipdrone
    ...
    ;──═◙ Change direction of the landscape scrolling based on the direction you're moving the helicopter
    ...
    if joy0right then tank1_xpos=tank1_xpos-4:tank2_xpos=tank2_xpos-3:tank3_xpos=tank3_xpos-2:radar1_xpos=radar1_xpos-2
    if joy0left then tank1_xpos=tank1_xpos+4:tank2_xpos=tank2_xpos+3:tank3_xpos=tank3_xpos+2:radar1_xpos=radar1_xpos+2

    ;──═◙ Move the cruise missiles on the screen
    gosub move_missiles bank2
...
mainLoopSkipUpdateText
    ...
    ;if battery_flag=1 then battery_ypos=battery_ypos+2:gosub sprite_battery

    ;──═◙ Subroutine for drawing all on-screen sprites.  It won't fit in bank 1.
    gosub sprite_draw bank2

    ;──═◙ Plot the city defense beam
    ; You cannot fire your laser when the city defense beam is active
    if beam_counter<30 then gosub draw_defensebeam bank2:playsfx sfx_plainlaser:goto skiplaserfire

    ;──═◙ Plot the laser when you press the fire button
    if laserLo=$00 && fire_debounce_1=1 then playsfx sfx_no_ammo:goto skiplaserfire
    if drone_ypos<22 then goto skiplaserfire ; If you're too high up you can't fire
    if laserLo<>0 && fire_debounce_1=1 then playsfx sfx_pulsecannon:gosub draw_laser bank2
...
skiplaserfire
    ...
    ;──═◙ Detect Hit with Battery powerup, recharge to 80
    ;if boxcollision(drone_xpos,drone_ypos,16,8, battery_xpos,battery_ypos,8,6) then healthLo=$80:battery_xpos=200:battery_ypos=0:playsfx sfx_powerup

    ;──═◙ Detect Hit with Laser Powerup/Recharge Sprite
    ;if boxcollision(laser_powerup_xpos,laser_powerup_ypos,8,8, drone_xpos,drone_ypos,16,8) then laserLo=$30:playsfx sfx_powerup
    if laser_powerup_xpos>drone_xpos && laser_powerup_xpos<(drone_xpos+16) && laser_powerup_ypos>drone_ypos && laser_powerup_ypos<(drone_ypos+8) then laserLo=$30:playsfx sfx_powerup
    ...
    ;-----------------------------------------------
    ;if boxcollision(crosshair_x_offset_1,laser_y_offset_10,4,4, tank1_xpos,tank1_ypos,16,8) then tank1_flag=1:animate3=0: gosub sfx_tanks
    ;if boxcollision(crosshair_x_offset_1,laser_y_offset_10,4,4, tank2_xpos,tank2_ypos,16,8) then tank2_flag=1:animate3=0: gosub sfx_tanks
    ;if boxcollision(crosshair_x_offset_1,laser_y_offset_10,4,4, tank3_xpos,tank3_ypos,16,8) then tank3_flag=1:animate3=0: gosub sfx_tanks
    ;-----------------------------------------------
    if crosshair_x_offset_1>tank1_xpos && crosshair_x_offset_1<(tank1_xpos+20) && laser_y_offset_10>tank1_ypos && laser_y_offset_10<(tank1_ypos+12) then tank1_flag=1:animate3=0: gosub sfx_tanks
    if crosshair_x_offset_1>tank2_xpos && crosshair_x_offset_1<(tank2_xpos+20) && laser_y_offset_10>tank2_ypos && laser_y_offset_10<(tank2_ypos+12) then tank2_flag=1:animate4=0: gosub sfx_tanks
    if crosshair_x_offset_1>tank3_xpos && crosshair_x_offset_1<(tank3_xpos+20) && laser_y_offset_10>tank3_ypos && laser_y_offset_10<(tank3_ypos+12) then tank3_flag=1:animate5=0: gosub sfx_tanks
    ;-----------------------------------------------
    ;if tank1_xpos>crosshair_x_offset_1 && tank1_xpos<(crosshair_x_offset_1+8) && tank1_ypos>laser_y_offset_10 && tank1_ypos<(laser_y_offset_10+8) then tank1_flag=1:animate3=0: gosub sfx_tanks
    ;if tank2_xpos>crosshair_x_offset_1 && tank2_xpos<(crosshair_x_offset_1+8) && tank2_ypos>laser_y_offset_10 && tank2_ypos<(laser_y_offset_10+8) then tank2_flag=1:animate3=0: gosub sfx_tanks
    ;if tank3_xpos>crosshair_x_offset_1 && tank3_xpos<(crosshair_x_offset_1+8) && tank3_ypos>laser_y_offset_10 && tank3_ypos<(laser_y_offset_10+8) then tank3_flag=1:animate3=0: gosub sfx_tanks

skipcoll1

    ;──═◙ Detect hits with cruise missiles
    ;if boxcollision(drone_xpos,drone_ypos,16,8, missile1_xpos,missile1_ypos,16,8) then dec scored=scored-1:gosub sfx_missiles
    ;if boxcollision(drone_xpos,drone_ypos,16,8, missile1_xpos,missile2_ypos,16,8) then dec scored=scored-1:gosub sfx_missiles
    ;if boxcollision(drone_xpos,drone_ypos,16,8, missile1_xpos,missile3_ypos,16,8) then dec scored=scored-1:gosub sfx_missiles
    ;-----------------------------------------------
    ;if drone_xpos>missile1_xpos && drone_xpos<(missile1_xpos+18) && drone_ypos>missile1_ypos && drone_ypos<(missile1_ypos+8) then dec scored=scored-1:gosub sfx_missiles
    ;if drone_xpos>missile2_xpos && drone_xpos<(missile2_xpos+18) && drone_ypos>missile2_ypos && drone_ypos<(missile2_ypos+8) then dec scored=scored-1:gosub sfx_missiles
    ;if drone_xpos>missile3_xpos && drone_xpos<(missile3_xpos+18) && drone_ypos>missile3_ypos && drone_ypos<(missile3_ypos+8) then dec scored=scored-1:gosub sfx_missiles
    ;-----------------------------------------------
    if missile1_xpos>drone_xpos && missile1_xpos<(drone_xpos+17) && missile1_ypos>drone_ypos && missile1_ypos<(drone_ypos+9) then dec scored=scored-1:gosub sfx_missiles
    if missile2_xpos>drone_xpos && missile2_xpos<(drone_xpos+17) && missile2_ypos>drone_ypos && missile2_ypos<(drone_ypos+9) then dec scored=scored-1:gosub sfx_missiles
    if missile3_xpos>drone_xpos && missile3_xpos<(drone_xpos+17) && missile3_ypos>drone_ypos && missile3_ypos<(drone_ypos+9) then dec scored=scored-1:gosub sfx_missiles

    ;──═◙ Detect hits with City Defense Beam 
    if beam_counter>29 then goto DefenseBeamEnd ; Don't check for a hit when the beam is not on the screen
    if defensebeam_x_offset_1>drone_xpos && defensebeam_x_offset_1<(drone_xpos+16) && defensebeam_y_offset_1>drone_ypos && defensebeam_y_offset_1<(drone_ypos+8) then dec scored=scored-1:gosub sfx_defense_beam:goto DefenseBeamEnd
    if defensebeam_x_offset_1>drone_xpos && defensebeam_x_offset_1<(drone_xpos+16) && defensebeam_y_offset_2>drone_ypos && defensebeam_y_offset_2<(drone_ypos+8) then dec scored=scored-1:gosub sfx_defense_beam:goto DefenseBeamEnd
    if defensebeam_x_offset_1>drone_xpos && defensebeam_x_offset_1<(drone_xpos+16) && defensebeam_y_offset_3>drone_ypos && defensebeam_y_offset_3<(drone_ypos+8) then dec scored=scored-1:gosub sfx_defense_beam:goto DefenseBeamEnd
    if defensebeam_x_offset_1>drone_xpos && defensebeam_x_offset_1<(drone_xpos+16) && defensebeam_y_offset_4>drone_ypos && defensebeam_y_offset_4<(drone_ypos+8) then dec scored=scored-1:gosub sfx_defense_beam:goto DefenseBeamEnd
    if defensebeam_x_offset_1>drone_xpos && defensebeam_x_offset_1<(drone_xpos+16) && defensebeam_y_offset_5>drone_ypos && defensebeam_y_offset_5<(drone_ypos+8) then dec scored=scored-1:gosub sfx_defense_beam:goto DefenseBeamEnd
    if defensebeam_x_offset_1>drone_xpos && defensebeam_x_offset_1<(drone_xpos+16) && defensebeam_y_offset_6>drone_ypos && defensebeam_y_offset_6<(drone_ypos+8) then dec scored=scored-1:gosub sfx_defense_beam:goto DefenseBeamEnd
    if defensebeam_x_offset_1>drone_xpos && defensebeam_x_offset_1<(drone_xpos+16) && defensebeam_y_offset_7>drone_ypos && defensebeam_y_offset_7<(drone_ypos+8) then dec scored=scored-1:gosub sfx_defense_beam:goto DefenseBeamEnd
    if defensebeam_x_offset_1>drone_xpos && defensebeam_x_offset_1<(drone_xpos+16) && defensebeam_y_offset_8>drone_ypos && defensebeam_y_offset_8<(drone_ypos+8) then dec scored=scored-1:gosub sfx_defense_beam:goto DefenseBeamEnd
    if defensebeam_x_offset_1>drone_xpos && defensebeam_x_offset_1<(drone_xpos+16) && defensebeam_y_offset_9>drone_ypos && defensebeam_y_offset_9<(drone_ypos+8) then dec scored=scored-1:gosub sfx_defense_beam:goto DefenseBeamEnd
DefenseBeamEnd
...
CopyTextZoneContentToRam
    asm
    SET_POINTER pointer,textZoneContent
end
    ...
    ;──═◙ On-Screen Text Data
    alphadata textZoneContent font2
    '                                        '
    'destroy enemy tanks!                                                                                          '
...
exitIncrementPointerBy
 ...
 data sfx_simplebuzz ; bass bump
    $10,$14,$00 ; version, priority, frames per chunk
...
game_over_loop
    ...
    if player_flag=2 then plotsprite text_gameover 1 60 40
...
level_up_loop
    ...
    restorescreen
    plotsprite text_levelup_top 0 32 40
    plotsprite text_levelup_bottom 0 32 48
    gosub sprite_draw bank2
    ...
    if joy0fire then debounce_fire=9
    if !joy0fire && debounce_fire=9 then debounce_fire=10
    if debounce_fire=10 then debounce_fire=0:return
...
Init
    ...
    missile3_xpos=185 :  missile3_ypos=75
    laser_powerup_xpos=45:laser_powerup_ypos=10
    battery_xpos=30   :  battery_ypos=0
    ...
    laserHi = $00  : laserMed = $00  : laserLo = $30
...
CheckValue
  ;   INPUT:  ValueCheckHi, ValueCheckMd, ValueCheckLo
  ;   OUTPUT: If Value > ValueCheck then ValueCompare=1 
 asm 
  ;   A comparison which branches to SetValueFlag if ValueCheck (scorea) > Value (scoreb)
  ;   scoreb is the 24 bit value you are changing, scorea is the static value you are comparing against.
           LDA ValueCheckHi            
           CMP ValueHi                  ; COMPARE HIGH BYTES 
           BCC SkipValueCheck           ; if ValueCheckHi < ValueHi then ValueCheck < Value
           BNE SetValueFlag             ; if ValueCheckHi <> ValueHi then ValueCheck > Value (so ValueCheck >= Value)
           LDA ValueCheckMd             
           CMP ValueMd                  ; COMPARE MIDDLE BYTES
           BCC SkipValueCheck           ; if ValueCheckMd < ValueMd then ValueCheck < Value
           BNE SetValueFlag             ; if ValueCheckMd <> ValueMd then ValueCheck > Value (so ValueCheck >= Value)
           LDA ValueCheckLo             
           CMP ValueLo                  ; COMPARE LOW BYTES
           BCS SkipValueCheck           ; if ValueCheckLo >= ValueLo then ValueCheck >= Value
SetValueFlag
