...
mainLoopSkipUpdateText
 ...
 if CARRY then laserLo=$30:gosub sfx_powerup bank2
...
skipcoll1
 ...
 if CARRY then dec scored=scored-1:gosub sfx_missiles
    ...
    if beam_counter>29 then goto DefenseBeamEnd
    asm
...
DefenseBeamEnd
    ...
    if beam_counter<30 then gosub draw_defensebeam bank2:gosub sfx_plainlaser bank2:goto skiplaserfire
    ...
    if laserLo<>0 && fire_debounce_1=1 then gosub sfx_pulsecannon bank2:gosub draw_laser bank2
...
game_over_loop
    ...
    if player_flag=2 then     plotsprite text_gameover_top 0 24 40:    plotsprite text_gameover_bottom 0 24 48  ;plotsprite text_gameover 1 60 40
...
cannon_fire
   ...
   ;│ Code                              │   sprite               │ palette │      X       │      Y       │ animation frame │   Notes                                 │
   ;└───────────────────────────────────┴────────────────────────┴─────────┴──────────────┴──────────────┴─────────────────┴─────────────────────────────────────────┘
   plotsprite                              laser                      5      cannon_fire_xpos  cannon_fire_ypos
   return
