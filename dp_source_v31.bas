    ;──═◙ "Drone Patrol", Steve Engelhardt, 3/9/2023
...
skiphealthcheck
    ...
    if beam_counter<50 then cannon_fire_ypos=cannon_fire_ypos-2 
    if beam_counter>49 || cannon_fire_ypos<6 then cannon_fire_ypos=163
...
mainLoopSkipUpdateText
    ...
    if laserLo=$00 && fire_debounce_1=1 then playsfx sfx_simplebuzz:goto skiplaserfire
...
skiplaserfire
    ...
    if gamedifficulty>1 then gosub cannon_fire bank2
    ...
    if cannon_fire_xpos>drone_xpos && cannon_fire_xpos<(drone_xpos+16) && cannon_fire_ypos>drone_ypos && cannon_fire_ypos<(drone_ypos+8) then dec scored=scored-49:gosub sfx_defense_beam
...
cannon_fire
   ...
   ;│ Code                              │   sprite               │ palette │      X       │      Y       │ animation frame │   Notes                                 │
   ;└───────────────────────────────────┴────────────────────────┴─────────┴──────────────┴──────────────┴─────────────────┴─────────────────────────────────────────┘
   plotsprite                              laser                      beam_counter      cannon_fire_xpos  cannon_fire_ypos
   return
